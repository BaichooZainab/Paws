...
function sendEmail()
{
    ...
    try {
        ...
        $mail->isHTML(TRUE);
        $mail->Body = '<html>Your account has been activated, you may now join PAWS. Click on the link to <strong><a href="http://localhost/oswt/pawcopy/orglogin.php">login</a></strong>.</html>';
        $mail->AltBody = 'In case the HTML does not work.';

        $fn = $_FILES['org_profile']['name'];
