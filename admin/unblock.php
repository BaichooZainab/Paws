...
if (isset( $_GET["did"] )) {
    ...
    $sql = " update tbldonator set d_status = 1 where d_id=:did";
    $stmt = $pdo->prepare($sql);
    //retrieve the sid from the url
    $stmt->execute(array(':did' => $_GET["did"] ));
    $_SESSION['successmsg'] = 'Donator Unblocked';
    ...
    if (isset( $_GET["aid"] )) {
        try {
        //add the delete statement with a where clause
        $sql = " update tbladopter set a_status = 1 where a_id=:aid";
        $stmt = $pdo->prepare($sql);
        //retrieve the sid from the url
        $stmt->execute(array(':aid' => $_GET["aid"] ));
        $_SESSION['successmsg'] = 'Adopter Unblocked';
